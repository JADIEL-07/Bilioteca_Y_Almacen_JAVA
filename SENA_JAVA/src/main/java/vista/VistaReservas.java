...
public class VistaReservas extends JPanel {
    ...
    public void mostrarDisponibilidad(boolean existe, int disponibles, String fechaLibre) {
        ...
        if (disponibles > 0) {
            lblDisponibilidad.setText("✅ Disponible. Puede reservar desde hoy.");
            lblDisponibilidad.setForeground(new Color(34, 197, 94)); // Verde
            btnGuardar.setEnabled(true);
        } else {
            String msg = "⚠️ Elemento en uso.";
            if (fechaLibre != null && !fechaLibre.isEmpty()) {
                msg += " Reserva sugerida desde: " + fechaLibre;
                txtFechaReserva.setText(fechaLibre);
            }
            lblDisponibilidad.setText(msg);
            lblDisponibilidad.setForeground(new Color(234, 179, 8)); // Amarillo (Warning)
            btnGuardar.setEnabled(true); // Se permite reservar para el futuro
        }
    }
    ...
    private void crearDialogoEditar() {
        dialogoEditar = new JDialog((Frame) null, "Editar Reserva", true);
        ...
        JLabel titulo = new JLabel("Actualizar Estado de Reserva");
        ...
        cbEditEstado = new JComboBox<>(new String[]{"PENDING", "CLAIMED", "COMPLETED", "CANCELLED", "EXPIRED"});
